...
var require_path_key = __commonJS({
  "node_modules/.bun/path-key@3.1.1/node_modules/path-key/index.js"(exports2, module2) {
    ...
    var pathKey2 = (options = {}) => {
      const environment6 = options.env || process.env;
      const platform2 = options.platform || process.platform;
      if (platform2 !== "win32") {
        return "PATH";
      }
      return Object.keys(environment6).reverse().find((key) => key.toUpperCase() === "PATH") || "Path";
    };
...
var { supportPath: supportPath2 } = import_api11.environment;
...
var VaultListenersContext = (0, import_react5.createContext)(null);
...
var VaultContext = (0, import_react8.createContext)(null);
var { syncOnLaunch } = (0, import_api20.getPreferenceValues)();
...
var SessionStorage = {
  getSavedSession: () => {
    return Promise.all([
      import_api26.LocalStorage.getItem(LOCAL_STORAGE_KEY.SESSION_TOKEN),
      import_api26.LocalStorage.getItem(LOCAL_STORAGE_KEY.REPROMPT_HASH),
      import_api26.LocalStorage.getItem(LOCAL_STORAGE_KEY.LAST_ACTIVITY_TIME),
      import_api26.LocalStorage.getItem(LOCAL_STORAGE_KEY.VAULT_LAST_STATUS)
    ]);
  },
  clearSession: async () => {
    await Promise.all([
      import_api26.LocalStorage.removeItem(LOCAL_STORAGE_KEY.SESSION_TOKEN),
      import_api26.LocalStorage.removeItem(LOCAL_STORAGE_KEY.REPROMPT_HASH)
    ]);
  },
  saveSession: async (token, passwordHash) => {
    await Promise.all([
      import_api26.LocalStorage.setItem(LOCAL_STORAGE_KEY.SESSION_TOKEN, token),
      import_api26.LocalStorage.setItem(LOCAL_STORAGE_KEY.REPROMPT_HASH, passwordHash)
    ]);
  },
  logoutClearSession: async () => {
    await Promise.all([
      import_api26.LocalStorage.removeItem(LOCAL_STORAGE_KEY.SESSION_TOKEN),
      import_api26.LocalStorage.removeItem(LOCAL_STORAGE_KEY.REPROMPT_HASH),
      import_api26.LocalStorage.removeItem(LOCAL_STORAGE_KEY.LAST_ACTIVITY_TIME)
    ]);
  }
};
...
var SessionContext = (0, import_react12.createContext)(null);
...
async function logoutVaultCommand() {
  try {
    const hasConfirmed = await (0, import_api28.confirmAlert)({
      title: "Logout From Bitwarden Vault",
      message: "Are you sure you want to logout from your current vault account?",
      icon: import_api28.Icon.Logout,
      primaryAction: { title: "Confirm", style: import_api28.Alert.ActionStyle.Destructive }
    });
    if (!hasConfirmed) return;
    const toast = await (0, import_api28.showToast)(import_api28.Toast.Style.Animated, "Logging out...");
    const bitwarden = await new Bitwarden(toast).initialize();
    const { error } = await bitwarden.logout();
    if (error instanceof NotLoggedInError) {
      toast.style = import_api28.Toast.Style.Failure;
      toast.title = "No session found";
      toast.message = "You are not logged in";
      return;
    }
  } catch (error) {
    await (0, import_api28.showToast)(import_api28.Toast.Style.Failure, "Failed to logout from vault");
  }
  try {
    await cleanupPostLogout();
    await (0, import_api28.showToast)(import_api28.Toast.Style.Success, "Successfully logged out");
  } catch (error) {
    await (0, import_api28.showToast)(import_api28.Toast.Style.Failure, "Failed to logout from vault");
  }
}
