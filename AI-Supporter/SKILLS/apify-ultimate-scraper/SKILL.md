...
```bash
node reference/scripts/search_actors.js --query "SEARCH_KEYWORDS"
```
...
```bash
node --env-file=.env reference/scripts/fetch_actor_details.js --actor "ACTOR_ID"
```
...
```bash
node --env-file=.env reference/scripts/run_actor.js \
  --actor "ACTOR_ID" \
  --input 'JSON_INPUT'
```
