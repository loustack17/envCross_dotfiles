...
M._defaults = {
    ...
    providers = {
        ...
        vertex = {
            endpoint =
            "https://aiplatform.googleapis.com/v1/projects/PROJECT_ID/locations/LOCATION/publishers/google/models",
    ...
    if file then
        ...
        if data.last_provider then
            ...
            if not provider then
                Utils.warn(
                    "Provider " ..
                    data.last_provider .. " is no longer a valid provider, falling back to default configuration"
                )
                os.remove(storage_path)
                return
            end
